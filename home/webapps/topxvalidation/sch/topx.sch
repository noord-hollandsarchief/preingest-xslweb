...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
	...
	<pattern id="topx-pattern-1a">
		<rule context="topx:aggregatie | topx:bestand">
			...
			<assert test="count(topx:naam) eq 0">Het "naam"-gegeven ontbreekt in de ToPX-metadata</assert>
		</rule>
	</pattern>
	<pattern id="topx-pattern-1b">
		<rule context="topx:aggregatie | topx:bestand">
			<p>Controleer of er precies één element "naam" is (dus niet 0 of meer dan 1)</p>
			<report test="count(topx:naam) gt 1">Er is meer dan één "naam"-gegeven in de ToPX-metadata</report>
			<assert test="count(topx:naam) gt 1">Er is meer dan één "naam"-gegeven in de ToPX-metadata</assert>
		</rule>
	</pattern>
	<pattern id="topx-pattern-1c">
		<rule context="topx:aggregatie | topx:bestand">
			...
			<assert test="count(topx:classificatie/topx:omschrijving) eq 0">Het "classificatie/omschrijving"-gegeven ontbreekt in de ToPX-metadata</assert>
		</rule>
	</pattern>
	<pattern id="topx-pattern-1d">
		<rule context="topx:aggregatie | topx:bestand">
			<p>Controleer of er precies één element "omschrijving" is (dus niet 0 of meer dan 1)</p>
			...
			<assert test="count(topx:classificatie/topx:omschrijving) gt 1">Er is meer dan één "classificatie/omschrijving"-gegeven in de ToPX-metadata</assert>
		</rule>
	</pattern>
	<pattern id="topx-pattern-2a">
		<rule context="topx:aggregatie">
			...
			<assert test="count(topx:openbaarheid/topx:omschrijvingBeperkingen) eq 0">Het "openbaarheid/omschrijvingBeperkingen"-gegeven ontbreekt in de ToPX-metadata</assert>
		</rule>
	</pattern>
	<pattern id="topx-pattern-2b">
		<rule context="topx:aggregatie">
			...
			<p>Controleer de aanwezigheid van 1 "omschrijvingBeperkingen"</p>
			<assert test="count(topx:openbaarheid/topx:omschrijvingBeperkingen) gt 1">Er is meer dan één "openbaarheid/omschrijvingBeperkingen"-gegeven in de ToPX-metadata</assert>
		</rule>
	</pattern>
	<pattern id="topx-pattern-2c">
